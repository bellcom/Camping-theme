...
function camping_preprocess_html(&$variables) {
  ...
  if (theme_get_setting('camping_enable_active_map')) {
    $variables['classes_array'][] = 'active-map-enabled';
  }
}
 ...
 */
function camping_preprocess_node(&$variables) {
 ...
 */
function camping_preprocess_page(&$variables) {

  // Social Block Settings.
  $variables['facebook_url'] = check_url(theme_get_setting('facebook_url', 'camping'));
  $variables['twitter_url'] = check_url(theme_get_setting('twitter_url', 'camping'));
  $variables['gplus_url'] = check_url(theme_get_setting('gplus_url', 'camping'));
  $variables['pinterest_url'] = check_url(theme_get_setting('pinterest_url', 'camping'));

  // Social Icons path.
  $variables['path'] = base_path() . path_to_theme('theme', 'camping');

  // Carousel Settings.
  $carousel_enabled = theme_get_setting('carousel_enabled', 'camping');
  if ($carousel_enabled) {
    $slide_count = check_plain(theme_get_setting('slide_count', 'camping'));
    $variables['arrow_enabled'] = theme_get_setting('arrow_enabled', 'camping');
    $variables['bullets_enabled'] = theme_get_setting('bullets_enabled', 'camping');

    $carousel_items = array();
    // Range indexes start's with 0, hence the slide_count+1 in the for
    // condition.
    for ($index = 1; $index <= $slide_count + 1; $index++) {
      $carousel_image = theme_get_setting('carousel_image_' . $index, 'camping');
      if (!empty($carousel_image)) {
        $carousel_image = file_load($carousel_image);

        $carousel_items[$index] = array(
          'carousel_image' => $carousel_image,
          'carousel_alt_text' => check_plain(strip_tags(theme_get_setting('carousel_alt_text_' . $index, 'camping'))),
          'carousel_caption' => check_plain(strip_tags(theme_get_setting('slide_caption_' . $index, 'camping'))),
        );
      }
    }
 ...
 */
function camping_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function camping_item_list($vars) {
 ...
 */
function camping_breadcrumb($variables) {
 ...
 */
function camping_links__system_secondary_menu($variables) {
 ...
 */
function camping_links__system_main_menu($variables) {
  $links = $variables['links'];
  $attributes = $variables['attributes'];
  $heading = $variables['heading'];
  global $language_url;
 ...
 */
function _camping_load_bootstrap() {
  ...
  _camping_add_asset('js', $bootstrap_path . $js_path, $js_options);
  _camping_add_asset('css', $bootstrap_path . $css_path, $css_options);
  if (isset($cssr_path)) {
    _camping_add_asset('css', $bootstrap_path . $cssr_path, $css_options);
  }
}
 ...
 * Adds js/css file.
 */
function _camping_add_asset($type, $data, $options) {
  if (isset($options['browsers']) && !is_array($options['browsers'])) {
    $options['browsers'] = _camping_browsers_to_array($options['browsers']);
  }
  switch ($type) {
    case 'css':
    ...
    case 'js':
      if (isset($options['browsers'])) {
        ...
        _camping_add_html_head_bottom(drupal_render($elements));
      }
      else {
 ...
 */
function _camping_browsers_to_array($browsers) {
  switch ($browsers) {
    case 'modern':
 ...
 */
function _camping_add_html_head_bottom($data = NULL) {
 ...
 */
function camping_page_alter($page) {
  $viewport = array(
    '#type' => 'html_tag',
    '#tag' => 'meta',
    '#attributes' => array(
